	...
	xfs_buf_t	*bp)
{
	...
	XFS_BUF_SET_FLAGS(bp, (B_READ|B_BUSY));
	...
	xlog_rec_header_t	*head)
{
	int			b;
#ifndef __FUNCTION__
#define __FUNCTION__	"xlog_header_check_dump"
#endif

	printf("%s:  SB : uuid = ", __FUNCTION__);
	for (b = 0; b < 16; b++)
		printf("%02x",((unsigned char *)&mp->m_sb.sb_uuid)[b]);
	printf(", fmt = %d\n", XLOG_FMT);
	printf("    log : uuid = ");
	for (b = 0; b < 16; b++)
		printf("%02x",((unsigned char *)&head->h_fs_uuid)[b]);
	...
	int			readonly)
{
	...
	*tail_blk = BLOCK_LSN(rhead->h_tail_lsn, ARCH_CONVERT);
	 ...
	 */
	if (!xfs_readonly_buftarg(&log->l_mp->m_logdev_targ)) {
