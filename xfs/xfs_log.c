...
xlog_iodone(xfs_buf_t *bp)
{
	...
	/*
	 * This is just not signalling iodone but rather completing all the
	 * unfinished semantics of biodone.
	 * Other places call back to biodone but not done here?
	 */
	...
	/*
	 * Reenable B_CALL flag if biodone cleared it
	 */
	XFS_BUF_CALL(bp);
			   ...
			   xlog_t	*log)
{
...
#endif
	{
		 ...
		 */
		if (mp->m_logbufs == -1) { 
			if (physmem <= btoc(128*1024*1024)) { 
				log->l_iclog_bufs = XLOG_MIN_ICLOGS; 
			} else if (physmem <= btoc(400*1024*1024)) { 
