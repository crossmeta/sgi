	 ...
	 */
		...
		if ((vap->va_size == 0) && 
		   (ip->i_d.di_size == 0) && (ip->i_d.di_nextents == 0)) {
			...
			vnode_pager_setsize(vp, vap->va_size, vap->va_size);
			lock_flags &= ~XFS_ILOCK_EXCL;
			if (mask & AT_CTIME)
	...
	cred_t		*credp)
{
	 ...
	 */
	realvp = src_vp;
	error = VOP_REALVP(src_vp, &realvp);
	if (!error) {
                src_vp = realvp;
	}
 ...
 * fid structure (like xfs_fast_fid) but uses a 32 bit inode number.
 */
STATIC int
xfs_fid(
	bhv_desc_t	*bdp,
	fid_t		*fidp)
{
	...
	vn_trace_entry(BHV_TO_VNODE(bdp), "xfs_fid",
		       (inst_t *)__return_address);
	ASSERT(sizeof(fid_t) >= sizeof(xfs_fid_t));
	...
	xfid->xfs_fid_len = sizeof(xfs_fid_t) - sizeof(xfid->xfs_fid_len);
	xfid->xfs_fid_pad = 0;
	xfid->xfs_fid_ino = ip->i_ino;
	xfid->xfs_fid_gen = ip->i_d.di_gen;	
...
xfs_rwlock(
	...
	vrwlock_t	locktype)
{
	return xfs_rwlockf(bdp, locktype, 0);
}
	...
	int		flags)
{
	...
	case VRWLOCK_READ:
	case VRWLOCK_WRITE_DIRECT:
		xfs_ilock(ip, XFS_IOLOCK_SHARED|flags);
		break;

	case VRWLOCK_TRY_READ:
		return xfs_ilock_nowait(ip, XFS_IOLOCK_SHARED|flags);

	case VRWLOCK_TRY_WRITE:
		return xfs_ilock_nowait(ip, XFS_IOLOCK_EXCL|flags);
	...
	int		attr_flags)
{
	...
	return error;
#endif
	printf("xfs_change_file_space: unimplemented\n");
	return (ENOSYS);
}
